...
public class UpdateTransaction extends Transaction {
     ...
     */
    public UpdateTransaction(TransactionId id, Replica owner, EpochPair startEpochPair, int index, int value, ActorRef destination, TransactionId writeTid) {
    ...
    public void start() throws IllegalStateException {
        ...
        } else if(replica.isCoordinator()) { // Case 3: Coordinator replica who received an UpdateMsg from a participant
            startAsCoordinator(replica);
        } else {
            throw new IllegalStateException("UpdateTransaction " + this.getId() + " is not able to start, invalid state or parameters.");
        }   
    }
    //////////////////////////////////////////
    ...
    protected void forwardToCoordinator(Replica replica){
        ...
        this.timeout = replica.getContext().system().scheduler().scheduleOnce(
    ...
    protected void coordinatorStateMachine(Replica replica, Msg msg) {
        if(msg instanceof UpdateAckMsg) {
            ...
            if(this.coordinatorAckCount >= (replica.getSystemNumberOfActors() / 2 + 1) && this.state == UpdateTransactionState.WAITING_ACK ){
                WriteOkMsg writeOk = new WriteOkMsg(this.getId(), this.startEpochPair, replica.getSelf(), this.index, this.value);

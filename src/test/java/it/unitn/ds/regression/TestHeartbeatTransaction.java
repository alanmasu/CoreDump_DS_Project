...
class TestHeartbeatTransaction {
    ...
    void coordinatorBroadcastsHeartbeatsPeriodically() {
        ActorSystem system = ActorSystem.create("coordinatorBroadcastHeartbeatsPeriodically");
        ...
        try {
            ...
            ActorRef coordinator =
                    system.actorOf(Replica.props(coordinatorId, 1, 5, heartbeatIntervalMillis), "coordinator");

            int initialHeartbeatTransactionSequence = 0;
            TransactionId expectedHeartbeatTransactionId =
                    new TransactionId(coordinator, initialHeartbeatTransactionSequence);

            Map<Integer, ActorRef> group = new HashMap<>();
            group.put(coordinatorId, coordinator);
            group.put(1, followerProbe.getRef());

            coordinator.tell(new InitSystem(group, coordinatorId), ActorRef.noSender());

            HeartbeatMsg firstHeartbeat = followerProbe.expectMsgClass(Duration.ofSeconds(2), HeartbeatMsg.class);
            ...
            HeartbeatMsg secondHeartbeat = followerProbe.expectMsgClass(Duration.ofSeconds(2), HeartbeatMsg.class);
    ...
    void heartbeatTickIsRoutedByTransactionId() {
        ActorSystem system = ActorSystem.create("heartbeatTickIsRoutedByTransactionId");
        ...
        try {
            ...
            TransactionId activeTransactionId = new TransactionId(coordinator, 0);

            TransactionId unknownTransactionId = new TransactionId(coordinator, 999);

            coordinator.tell(new InitSystem(group, coordinatorId), ActorRef.noSender());

            coordinator.tell(new HeartbeatTickMsg(unknownTransactionId, null, coordinator), ActorRef.noSender());

            followerProbe.expectNoMessage(Duration.ofMillis(200));

            coordinator.tell(new HeartbeatTickMsg(activeTransactionId, null, coordinator), ActorRef.noSender());

            HeartbeatMsg heartbeat = followerProbe.expectMsgClass(Duration.ofSeconds(2), HeartbeatMsg.class);

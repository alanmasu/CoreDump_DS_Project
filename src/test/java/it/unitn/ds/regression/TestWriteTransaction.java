...
class TestWriteTransaction {
    ...
    void testSimpleWriteTransaction() {
        client.tell(
                new AbstractClient.WriteRequest(TestsCommons.TEST_INDEX, TestsCommons.TEST_VALUE, replica),
                Actor.noSender());
        ...
        sys.system
                .scheduler()
                .scheduleOnce(
                        Duration.ofMillis(TestsCommons.getMaxUpdateDelay(sys) * 3),
                        replica,
        ...
        assertEquals(
                TestsCommons.TEST_VALUE, result.value, "The value written should be the same as the one requested");
        assertEquals(
                TestsCommons.TEST_INDEX, result.index, "The index written should be the same as the one requested");
    ...
    void testWriteTransactionTimeout() {
        ...
        probe.expectNoMessage(
                Duration.ofMillis(TestsCommons.getClientWriteTimeout(AbstractReplica.MAX_LATENCY, sys.getNNodes())));
